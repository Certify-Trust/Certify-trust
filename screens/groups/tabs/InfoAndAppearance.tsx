...
const AppearanceCard = ({ type }: { type: "certificate" | "badge" }) => (
  <div className="relative flex h-[333px] w-[327px] cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-[#D0D5DD] bg-[#F9FAFB] transition hover:border-[#7F56D9]">
    <button className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-sm bg-white shadow-sm">
    ...
    <div className="space-y-10 pb-10">
      ...
      <section className="space-y-5">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {skills.map((s) => (
                <span key={s} className="flex items-center gap-1 rounded-full bg-[#F9F5FF] px-3 py-1 text-xs font-medium text-[#6941C6]">
                  {s}
                  <button onClick={() => setSkills((prev) => prev.filter((x) => x !== s))} className="ml-1 hover:opacity-70">×</button>
                </span>
              ))}
            </div>
          )}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-[#D0D5DD] px-4 py-2.5">
            <input placeholder="Type here and then select a category from the list" className="flex-1 text-sm text-[#344054] placeholder:text-[#98A2B3] focus:outline-none" />
